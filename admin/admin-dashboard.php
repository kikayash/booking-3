...
<body class="admin-theme">
            ...
            <div class="col-md-9 col-lg-10">
                <div class="card shadow">
                    ...
                    <div class="card-body">
                        ...
                        <!-- Quick Search Buttons - ENHANCED -->
                        <div class="row mb-3">
    <div class="col-md-6">
        <div class="d-flex align-items-center gap-2">
            <small class="text-muted">Pencarian Cepat UKM:</small>
            <div class="dropdown">
                <button class="btn btn-sm btn-outline-primary dropdown-toggle" type="button" 
                        id="quickSearchDropdown" data-bs-toggle="dropdown" aria-expanded="false">
                    <i class="fas fa-filter me-1"></i>
                    <?php 
                    // Show current search if it matches UKM list
                    $ukms = ['WAPPIM', 'BASKET', 'KI', 'APOSTOLOS', 'KATOLIK', 'HIMPAS', 'JAS', 'KESENIAN', 'MENWA', 'MICC', 'BUDDHA', 'HINDU', 'BOLA'];
                    $currentUKM = '';
                    foreach ($ukms as $ukm) {
                        if (strtolower($search) === strtolower($ukm)) {
                            $currentUKM = $ukm;
                            break;
                        }
                    }
                    echo $currentUKM ? $currentUKM : 'Pilih UKM';
                    ?>
                </button>
                <ul class="dropdown-menu" aria-labelledby="quickSearchDropdown">
                    <li>
                        <h6 class="dropdown-header">
                            <i class="fas fa-users me-1"></i>UKM & Organisasi
                        </h6>
                    </li>
                    <?php
                    $ukm_categories = [
                        'Olahraga' => ['BASKET', 'HIMPAS', 'BOLA'],
                        'Rohani' => ['KI', 'APOSTOLOS', 'KATOLIK', 'BUDDHA', 'HINDU'],
                        'Akademik' => ['JAS', 'WAPPIM'],
                        'Seni & Budaya' => ['KESENIAN', 'MICC'],
                        'Kepemimpinan' => ['MENWA']
                    ];
                    
                    foreach ($ukm_categories as $category => $ukms):
                    ?>
                        <li><h6 class="dropdown-header text-muted"><?= $category ?></h6></li>
                        <?php foreach ($ukms as $ukm): 
                            $active_class = (strtolower($search) === strtolower($ukm)) ? 'active' : '';
                        ?>
                            <li>
                                <a class="dropdown-item <?= $active_class ?>" 
                                   href="?status=<?= $status ?>&search=<?= urlencode($ukm) ?>">
                                    <i class="fas fa-search me-2 text-primary"></i><?= $ukm ?>
                                    <?php if ($active_class): ?>
                    ...
                    <li><hr class="dropdown-divider"></li>
                    <li>
                        <a class="dropdown-item text-secondary" href="?status=<?= $status ?>">
            ...
            <?php if (!empty($search) && $currentUKM): ?>
                <span class="badge bg-primary">
                    Filter: <?= htmlspecialchars($search) ?>
                </span>
                <a href="?status=<?= $status ?>" class="btn btn-sm btn-outline-secondary">
    ...
    <div class="col-md-6 text-end">
        <small class="text-muted">
            ...
            Gunakan kotak pencarian di atas untuk pencarian bebas, atau dropdown untuk filter UKM
        </small>
    </div>
</div>
                        </div><br>
